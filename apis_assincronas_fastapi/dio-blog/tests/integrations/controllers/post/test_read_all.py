...
@pytest.mark.parametrize("published_total", [("on", 2), ("off", 1)])
...
async def test_read_posts_unauthorized_fail(client: AsyncClient):
    # Given
    params = {"published": "on", "limit": 1}

    # When
    response = await client.get(f"/posts/", params=params, headers={})

    # Then
    assert response.status_code == status.HTTP_401_UNAUTHORIZED

async def test_read_post_empty_parameters_fail(client: AsyncClient, access_token: str):
    # Given
    headers = {"Authorization": f"Bearer {access_token}"}

    # When
    response = await client.get(f"/posts/", params={}, headers=headers)
